...
# that is load-bearing rather than style. Stage 3 fast-forwards the
# workspace-root checkout — which is where this script is usually run from — so
# a promotion that changes promote.sh rewrites the file *while bash is reading
# it*. Bash reads a script lazily, by byte offset, so the running promotion
# then executes a spliced mixture of the old file and the new one. Observed on
# 2026-08-27: the run that introduced the trilium install fixup silently skipped
# that very block. A function definition is parsed in full before it is called,

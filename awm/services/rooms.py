...
def list_rooms(*, status: str | None = "open",
               participating_scope: str | None = None,
               limit: int = 100) -> list[Room]:
    """List rooms. ``status='active'`` is accepted as an alias for the
    legacy "open" rooms state to keep existing callers working."""
    if status == "active":
        status = "open"
    conn = get_connection()
    try:
...
SELECT DISTINCT r.*
 ...
 WHERE r.topic LIKE ? OR r.id LIKE ? OR t.body LIKE ?

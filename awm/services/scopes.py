...
def _default_context(project: str, scope: str) -> str:
    ...
    return (
        f"# {project}/{scope}\n\n"
        f"## Startup\n\n"
        f"1. Run `awm_refresh project={project} scope={scope}` to update local knowledge and artifact indexes\n"
        f"2. Read `.awm/knowledge.md` for gotchas and prior lessons\n"
        ...
        f"- Do NOT edit `.awm/knowledge.md` or `.awm/artifacts.md` — use MCP tools (see prefaces in those files)\n\n"
        f"## Debrief\n\n"
...
def _generate_knowledge_md(project: str, scope: str) -> str:
    """Generate knowledge.md content from experiences and skills for a project."""
    preface = (
        ...
        f"     To search: `skills_search query=\"...\"` or `session_list project={project}` -->\n\n"
        f"# Project Knowledge: {project}\n\n"
    )
    ...
    try:
        ...
        # Recent experiences for the project
        experiences = conn.execute(
            "SELECT * FROM experiences WHERE project=? ORDER BY created_at DESC LIMIT 50",
            (project,),
        ).fetchall()
    finally:
        conn.close()
    ...
    # Experiences grouped by skill
    if experiences:
        # Group by skill_path
        by_skill: dict[str, list] = {}
        for exp in experiences:
            key = exp["skill_path"] or "(freeform)"
            by_skill.setdefault(key, []).append(exp)

        lines = ["## Recent Experiences\n"]
        for skill_key, exps in by_skill.items():
            lines.append(f"### Skill: {skill_key}\n")
            for exp in exps[:5]:  # limit per skill
                outcome = f" [{exp['outcome']}]" if exp["outcome"] else ""
                lines.append(f"**{exp['scope']}**{outcome}: {exp['summary']}")
                if exp["deviations"]:
                    lines.append(f"- Deviations: {exp['deviations']}")
                if exp["suggestions"]:
                    lines.append(f"- Suggestions: {exp['suggestions']}")
                lines.append("")
        sections.append("\n".join(lines))

    if not sections:
        sections.append("*No knowledge entries yet. Experiences will appear here after agents log them via `experience_log`.*\n")
...
def refresh_knowledge(project: str, scope: str) -> str:
    ...
    (awm_dir / "knowledge.md").write_text(content)
    return content
...
def awm_refresh(project: str, scope: str) -> dict:
    """Refresh both knowledge.md and artifacts.md for a scope."""
    k = refresh_knowledge(project, scope)
    a = refresh_artifacts(project, scope)
    return {
        "message": f"Refreshed .awm/ for {project}/{scope}",
        "knowledge_lines": len(k.splitlines()),
...
def create_scope(req: ScopeCreateRequest) -> ScopeActionResponse:
    ...
    # 5. Generate knowledge.md and artifacts.md
    (awm_dir / "knowledge.md").write_text(_generate_knowledge_md(req.project, req.scope))

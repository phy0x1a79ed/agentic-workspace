...
def _row_to_scope_info(r) -> ScopeInfo:
    return ScopeInfo(
...
def search_scopes(
    ...
    sql = "SELECT project, scope, status, branch, worktree, repo_path, session FROM scopes WHERE 1=1"
    params: list = []
    ...
    sql += " ORDER BY project, scope LIMIT ? OFFSET ?"
    params.append(limit)
    ...
    keyword_hits = [_row_to_scope_info(r) for r in rows]
    ...
    def _materialize(source_id: str):
        ...
        try:
            extra_sql = (
                "SELECT project, scope, status, branch, worktree, repo_path, session "
                "FROM scopes WHERE project=? AND scope=?"
            )
            ...
            conn.close()
        return _row_to_scope_info(row) if row else None

"""Scope CRUD — create, complete, delete, list, refresh (v37).

A "scope" in v36 was a row in the ``scopes`` table; in v37 the same
identity lives on ``agents`` (status ∈ allocated|active|retired). This
module is the legacy entry point — public API stays the same so MCP and
CLI callers don't break — but the underlying queries are now against
``agents``/``projects``/``session_logs``/``artifacts`` joined via uuid
FKs.
"""
...
def _ensure_project_row(project: str, *, conn=None) -> str:
    ...
    return identity.ensure_project(project, repo_path=str(bare), conn=conn)
...
def _generate_history_md(project: str, scope: str) -> str:
    ...
    preface = (
        ...
        f"     To search: `session_search project={project} scope={scope}` (set status=all for resolved sessions) -->\n\n"
    ...
    try:
        ...
        open_sessions = conn.execute(
            "SELECT s.id, s.title, s.summary, a.scope, s.skill_path, "
            "       s.outcome, s.deviations, s.suggestions "
            "FROM session_logs s "
            "JOIN agents a ON a.id = s.agent_id "
            "JOIN projects p ON p.id = a.project_id "
            "WHERE p.name=? AND s.resolved_at IS NULL "
        ...
        resolved_sessions = conn.execute(
            ...
            "JOIN agents a ON a.id = s.agent_id "
            "JOIN projects p ON p.id = a.project_id "
            "WHERE p.name=? AND s.resolved_at IS NOT NULL "
            "ORDER BY s.resolved_at DESC LIMIT 50",
            (project,),
        ).fetchall()
    finally:
        conn.close()
...
def _generate_artifacts_md(project: str, scope: str) -> str:
    ...
    try:
        artifacts = conn.execute(
            "SELECT ar.*, p.name AS project_name, ag.scope AS scope_name "
            "FROM artifacts ar "
            "JOIN agents ag ON ag.id = ar.agent_id "
            "JOIN projects p ON p.id = ag.project_id "
            "WHERE p.name=? AND ar.status='current' "
            "ORDER BY ar.artifact_type, ar.name",
            (project,),
        ).fetchall()
    finally:
        conn.close()
    ...
    for atype, items in by_type.items():
        ...
        for a in items:
            ...
            fmt = a["format"] or ""
            lines.append(f"| {a['name']} | `{a['path']}` | {fmt} | {a['scope_name']} | {desc} |")
        sections.append("\n".join(lines))
...
def ensure_vagrant_repo() -> Path:
    ...
    # Ensure projects table has _vagrant
    conn = get_connection()
    try:
...
def ensure_vagrant_session(user_as: str) -> tuple[str, str]:
    ...
    agent_id = identity.agent_id_for_scope(VAGRANT_PROJECT, scope_name)
    if agent_id is None:
...
def create_scope(req: ScopeCreateRequest) -> ScopeActionResponse:
    ...
    try:
        ...
        # Session counter: count historical agents for this name + 1.
        prior = conn.execute(
        ...
        session_num = (prior[0] or 0) + 1
        active = identity.agent_id_for_scope(req.project, req.scope, conn=conn, active_only=True)
        if active:
    ...
    try:
        ...
        index_scope(req.project, req.scope)
    except Exception:
        pass
...
def update_scope(project: str, scope: str, req: ScopeUpdateRequest) -> ScopeActionResponse:
    ...
    aid = identity.agent_id_for_scope(project, scope, active_only=True)
    if aid:
        identity.retire_agent(aid)
...
def delete_scope(project: str, scope: str) -> ScopeActionResponse:
    ...
    try:
        ...
        delete_embedding("scope", f"{project}/{scope}")
    except Exception:
        pass
...
def _v37_render_scope(r) -> ScopeInfo:
    """Map a v37 agents-joined row to the legacy ScopeInfo shape.

    Status mapping: ``'allocated'`` and ``'active'`` both render as
    ``'active'`` (the legacy enum); ``'retired'`` → ``'completed'``;
    ...
    return ScopeInfo(
        ...
        repo_path=r["worktree"], session=r["session"] or 1,
    )
...
def search_scopes(
    ...
    """Search scopes. Defaults to status='active' (pass 'all' for the full
    history). When `query` is set, returns LIKE-matches on scope name first,
    then semantic top-up via the shared embeddings helper.

    v37: rows come from the ``agents`` table; ``allocated|active`` render as
    ``'active'``, ``retired`` renders as ``'completed'``.
    """
    conn = get_connection()
    try:
        sql = (
        ...
        if status and status != "all":
            if status == "active":
                ...
                # Legacy distinction lost in v37 — return nothing.
    ...
    keyword_hits = [_v37_render_scope(r) for r in rows]
    ...
    def _materialize(source_id: str):
        ...
        try:
            ...
            row = conn.execute(extra_sql, extra_params).fetchone()
        finally:
            conn.close()
        return _v37_render_scope(row) if row else None

    try:
        from awm.services.embeddings import hybrid_augment
        merged = hybrid_augment(
            query, source_type="scope",

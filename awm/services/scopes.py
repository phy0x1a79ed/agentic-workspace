...
def _generate_knowledge_md(project: str, scope: str) -> str:
    """Generate knowledge.md content from session logs and skills for a project."""
    preface = (
    ...
    try:
        ...
        # Recent sessions for the project
        recent_sessions = conn.execute(
            "SELECT * FROM session_logs WHERE project=? ORDER BY logged_at DESC LIMIT 50",
            (project,),
        ).fetchall()
    finally:
        conn.close()
    ...
    if not sections:
        sections.append("*No knowledge entries yet. Sessions will appear here after agents log them via `session_log`.*\n")

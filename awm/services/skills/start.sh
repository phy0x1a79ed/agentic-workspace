#!/usr/bin/env bash
# Entry point for the skills service. The hub spawns (and respawns) this
...
#
# Relies on `awm-skills` (and its awm-* component deps) being installed into
# the target env via ./install.sh — `python -m awm.skills.hub_adapter` then
...
exec "${AWM_PYTHON:-python}" -m awm.skills.hub_adapter

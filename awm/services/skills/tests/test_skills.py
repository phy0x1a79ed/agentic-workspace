"""Tests for awm.skills.skills — scanning, frontmatter, search, sync."""
...
import awm.skills.skills as skills
...
class TestSearchSkillsNoQuery:
    ...
    def test_empty_skills_dir(self, awm_workspace, monkeypatch):
        ...
        monkeypatch.setattr("awm.skills.skills.SKILLS_DIR", empty)
        result = skills.search_skills()
        assert result.total == 0
...
class TestSyncSkills:
    ...
    def _stub_index_skill(self, monkeypatch):
        monkeypatch.setattr(
            "awm.skills.skills.index_skill",
            lambda path: None,
        )

    @pytest.fixture(autouse=True)
    def _reset_dao_init(self):
        """Reset the DAO init flag so each test gets a fresh DB."""
        import awm.skills.dao as dao_mod
        dao_mod._initialized = False
        yield
        dao_mod._initialized = False

    def test_first_call_indexes_and_prunes(self, sample_skills_dir, skills_db_conn):
        # Seed a stale skill embedding whose file does not exist.
        skills_db_conn.execute(

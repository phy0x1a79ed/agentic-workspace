#!/usr/bin/env bash
# run.sh — self-contained entry point for the compute service (the watchdog).
#
...
#
# AWM_HUB_URL is load-bearing beyond registration here: the watchdog arms only
# when that URL's port is the production gateway's. Every dev sandbox
# bootstraps this service too, and sandboxes share one process table — the
# origin check is what stops a second armed watchdog appearing the moment
# someone starts a sandbox.
#
...
set -euo pipefail
cd "$(dirname "$0")"
MODULE="awm.compute.hub_adapter"

    ...
    *,
    ...
    if cur.fetchone() is None:
        conn.execute(
            ...
            " state, first_seen, last_seen) VALUES (?,?,?,?,?,?,?,?)",
            (session_id, harness, cwd, _project_label(cwd), title,
             state or "working", now, now),
        )
    else:
...
async def handle_report(conn: sqlite3.Connection, event: dict[str, Any]) -> dict[str, Any]:
    ...
    if ev == "session_start":
        ...
        delta.update(type="resolve" if ids else None, ids=ids)
        ...
        upsert_session(conn, session_id, harness, cwd=cwd, state="question")
        item, created = raise_item(
            conn, session_id, "question", _snippet(msg) or "Needs your input",
        ...
        if last is None and event.get("transcript_path"):
            last, tools = await classify.read_last_assistant(
                event["transcript_path"])
        question = classify.is_question(last, tool_names=tools)
        kind = "question" if question else "idle"
        upsert_session(conn, session_id, harness, cwd=cwd, state=kind)
        item, created = raise_item(
            conn, session_id, kind,
            _snippet(last) or ("Agent asked a question" if question
                               else "Agent finished and is idle"),
            snippet=_snippet(last),
            grace=0.0 if question else IDLE_GRACE_S,
        )
        delta.update(type="raise" if created else "update", item=item)

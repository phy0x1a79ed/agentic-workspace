"""PTT (STT-only) feature service — package ``awm.ptt``.
...
control-WS adapter (``python -m awm.ptt.hub_adapter`` on
...
auth, ever. Browser-side, calls go to ``/svc/ptt/{fn,session}/*`` and the hub

...
Run via the per-dist runner (PYTHONPATH = the ptt dist root + components):

    awm/gateway/scripts/run-tests.sh ptt

or directly, pointing PYTHONPATH at this dist's source root:

    PYTHONPATH=awm/services/ptt mamba run -n awm \\
        python -m pytest awm/services/ptt/awm/ptt/tests/test_convo.py -q
"""
...
from awm.ptt.convo.cleanup import CleanupError
from awm.ptt.convo.session import ConvoResult, ConvoSession

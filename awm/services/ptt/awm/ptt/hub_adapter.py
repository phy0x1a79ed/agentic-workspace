"""Hub adapter for the ptt service (STT-only).

Boots the ptt service as a gateway-registered process on the shared
...
from awm.ptt.registry import run_ptt_ws_session

log = logging.getLogger("awm.ptt.hub_adapter")
...
async def _h_transcribe(args: dict) -> dict:
    ...
    from awm.ptt.stt import get_transcriber
...
class _BridgeWs:
    ...
    duck-typed WS surface ``run_ptt_ws_session`` drives — ``accept`` /
...
async def _run_stream_session(ctx: SessionContext) -> None:
    ...
    ``run_ptt_ws_session`` handler runs unchanged, and drives the per-user PTT
    ...
    try:
        ...
        log.warning("ptt stream bridge open failed: %s", exc)
        return
    adapter = _BridgeWs(bridge)
    try:
        await run_ptt_ws_session(adapter, user_as)
    except Exception:  # noqa: BLE001

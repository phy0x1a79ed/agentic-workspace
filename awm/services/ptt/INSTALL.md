# Installing the `ptt` service (STT-only)

A Python feature service in the `awm.ptt` namespace. It needs the `awm` conda
...
loop that used to live in `tts` was pruned too — **`ptt` owns STT now**.
...
## Controlling the service
    ...
    awm services start ptt            # start (idempotent)
    awm services restart ptt
    awm services enable ptt           # clear the disable flag

Or iterate against a running dev hub without evicting the base:

    awm dev shadow awm/services/ptt   # overlay this worktree's copy; Ctrl-C pops

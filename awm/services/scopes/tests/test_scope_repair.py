...
@pytest.fixture
def split_brain_scope(scopes_workspace):
...
def test_repair_backfills_missing_db_row(scopes_workspace, split_brain_scope):
...
def test_repair_is_idempotent(scopes_workspace, split_brain_scope):
    from awm.scopes import scopes
...
def test_repair_rejects_missing_worktree(scopes_workspace):
    from awm.scopes import scopes

    # Make a bare so the project check passes, but no worktree dir.
    bare = scopes_workspace["projects_dir"] / "p1" / ".bare"
    bare.mkdir(parents=True)
    with pytest.raises(FileNotFoundError, match="Worktree not found"):
        scopes.repair_scope("p1", "nope")


def test_repair_rejects_worktree_without_context(scopes_workspace, split_brain_scope):
    from awm.scopes import scopes
...
def test_repair_rejects_missing_bare(scopes_workspace):
    from awm.scopes import scopes

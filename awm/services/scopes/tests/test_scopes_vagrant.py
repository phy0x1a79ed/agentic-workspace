...
from awm.scopes.models import ProjectCreateRequest, ScopeCreateRequest
from awm.scopes import projects as projects_svc
from awm.scopes import scopes as scopes_svc
from awm.persistence.config_service import (
    get_config,
...
def _init_config_service(scopes_workspace, monkeypatch):
    """The config service DB (used by get/set_config) is initialized at
    daemon startup in production; stand it up against the tmp workspace so
    these tests can read/write config. The scopes-dist conftest only inits
    the scopes DB, not config."""
    import awm.persistence.config_service as cfg_mod
    monkeypatch.setattr(cfg_mod, "_initialized", False)
    cfg_mod._ensure_db()
...
def test_reserved_project_name_blocks_create_project(scopes_workspace):
...
def test_vagrant_repo_url_defaults_to_github_user(scopes_workspace):
    url = get_vagrant_scopes_repo_url()
    assert url.endswith("/vagrant-scopes.git")
    assert "github.com" in url


def test_vagrant_repo_url_config_roundtrip(scopes_workspace):
...
def test_create_scope_vagrant_without_bootstrap_raises_with_hint(scopes_workspace):

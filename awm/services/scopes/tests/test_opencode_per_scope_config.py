...
from awm.scopes.scopes import _write_scope_opencode_config


class TestWriteScopeOpencodeConfig:
    def test_creates_file_with_instructions(self, tmp_path, scopes_workspace):
        wt = tmp_path / "wt"
        awm = wt / ".awm"
    ...
    def test_inherits_workspace_mcp_servers(self, tmp_path, scopes_workspace):
        workspace_cfg = scopes_workspace["awm_dir"] / "mcp-opencode.json"
    ...
    def test_idempotent_does_not_duplicate_instructions(self, tmp_path, scopes_workspace):
        wt = tmp_path / "wt"
        awm = wt / ".awm"
        awm.mkdir(parents=True)
    ...
    def test_survives_corrupt_workspace_config(self, tmp_path, scopes_workspace):
        workspace_cfg = scopes_workspace["awm_dir"] / "mcp-opencode.json"

"""Tests for awm.scopes.projects helpers."""
...
from awm.scopes.projects import _find_template
...
def test_find_template_matches_by_stem(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("awm.scopes.projects.WORKSPACE_ROOT", tmp_path)
    monkeypatch.setattr("awm.scopes.projects.SKILLS_DIR", skills)
...
def test_find_template_workspace_overrides_package(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("awm.scopes.projects.WORKSPACE_ROOT", tmp_path)
    monkeypatch.setattr("awm.scopes.projects.SKILLS_DIR", package_skills)

...
def _cleanup_worktree(bare_dir: Path, worktree_dir: Path, feature_branch: str,
    ...
    Under the annex layer this had to publish the scope's data clone first and
    refuse when content would be lost, because a clone could hold the only copy
    of something. **That specific hazard is gone.** A scope's data is links into
    the shared cache and its pins are commits on a branch, so deleting a
    worktree unlinks names, never bytes — the cache object keeps its own link.

    One hazard survives, and it is now the *same* hazard on both sides: work
    that was never committed. An un-added chunk or an un-committed pin dies with
    the worktree exactly as un-committed code does. So the guard is simply
    ``git status``, which under this design covers data and code at once — which
    is the whole point of collapsing them onto one lever. ``force=True``
    overrides it.

    A mechanical wrinkle survives too, but a smaller one than under annex. DVC
    leaves *directories* writable and only the files read-only, and ``rmtree``
    needs write permission on the containing directory — so the tree deletes
    fine. Read-only *directories* only appear where a legacy annex clone is
    still sitting in the worktree mid-migration, which is what
    :func:`chmod_dirs_writable` is for. It deliberately never touches files: a
    materialised file shares its inode with the shared cache object, so
    ``chmod +w`` on it would unprotect that object for every other scope and
...
def _default_context(project: str, scope: str) -> str:
    ...
    return (
        ...
        f"3. Skim `.awm/artifacts.md` — how to discover and reuse sibling scopes' outputs; `artifact_search` when you need one\n\n"
        f"## Work\n\n"
...
def _generate_artifacts_md(project: str, scope: str) -> str:
    """Render `.awm/artifacts.md` as a *discoverability pointer*, not an index.

    Every session's startup ritual reads this file, so it must stay bounded: it
    tells an agent that reusable outputs may exist and how to find them, and
    deliberately does **not** inline the list (which would grow without bound and
    be force-read on every wake). The live list is queried on demand with
    `artifact_search`. Kept fully in-process — no cross-service call.
    """
    return (
        f"<!-- AUTO-GENERATED by AWM. Do NOT edit this file directly.\n"
        f"     Refresh: `awm_refresh project={project} scope={scope}` -->\n\n"
        f"# Artifacts — {project}\n\n"
        f"Sibling scopes in this project register **reusable outputs** — figures, datasets,\n"
        f"reports, models, scripts. This file is a pointer, not a list: it says such outputs\n"
        f"may exist and how to find them; it does not enumerate them (search does that, live).\n\n"
        f"**When to look** — before you regenerate an output or recompute a result, and whenever\n"
        f"you need data a sibling scope may already have produced. Reuse beats recompute.\n\n"
        f"**How to look**\n"
        f"- Find: `artifact_search project={project} query=\"...\"` "
        f"(also filter by `artifact_type=figure|dataset|report|model|script|other`)\n"
        f"- Retrieve: `artifact_get artifact_id=<id>` on a hit\n"
        f"- Add yours: `artifact_register project={project} scope={scope} "
        f"name=\"...\" path=\"...\" artifact_type=...`\n\n"
        f"*No search hits means nothing is registered yet — register your outputs so the next\n"
        f"scope can find them.*\n"
    )
...
def _heal_worktree(worktree_dir: Path, *, project: str, scope: str, dry_run: bool) -> dict:
    ...
    # migration path for scopes that pre-date the current layer: a scope still
    # on the legacy shared symlink — or on an old annex clone — is brought to
    # whatever its checkout now says the first time heal runs over it.
    actions["data"] = _heal_data(awm_dir, project=project, scope=scope, dry_run=dry_run)

    return actions


# `.awm/data` shapes heal has to recognise. All four are live simultaneously
# during the rollout, so the classifier is explicit rather than a chain of
# `if exists` guesses — a legacy annex clone in particular is a real directory
# that must NOT be mistaken for somebody's stray files and left alone forever.
...
def _heal_data(awm_dir: Path, *, project: str, scope: str, dry_run: bool) -> str | None:
    ...
    Four states are live at once during the rollout — a legacy shared symlink, a
    git-annex clone, the DVC compat symlink, and nothing at all — so this reports
    the transition it made rather than assuming a starting point.
    """
    dest = awm_dir / "data"
...
def gather_scope(project: str, hub: str, peripherals: list[str],
    ...
    There is no longer a ``data=True`` leg, and its absence is the point. Under
    the annex layer data had its own branch per scope, so fanning it in was a
    second, separate batch of merges that had to publish content to a canonical
    repo first — and therefore could not stay local-only. Now a peripheral's
    data pins ride its code branch, so **merging the branch merges the data**;
    the post-merge hook materialises it. One batch, still local-only.
    """
    validate_name(project, kind="project name")

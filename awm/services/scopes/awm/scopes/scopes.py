...
The rooms/sessions calls inside this module stay IN-PROCESS
(``awm.scopes.rooms`` / ``awm.scopes.sessions``), since all these
surfaces co-reside in the same dist.
"""
...
def _generate_history_md(project: str, scope: str) -> str:
    from awm.scopes.sessions import _display_title
    ...
    open_sessions = dao.query_all(
        "SELECT id, title, summary, scope, skill_path, outcome, deviations, suggestions "
        "FROM session_logs "
        "WHERE project=? AND resolved_at IS NULL "
        "ORDER BY created_at DESC LIMIT 50",
        (project,),
    )
    resolved_sessions = dao.query_all(
        "SELECT id, title, summary FROM session_logs "
    ...
    if not sections:
        sections.append(
            "*No session entries yet. Sessions will appear here after agents log them via `session_log`.*\n"
        )

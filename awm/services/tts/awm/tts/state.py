"""TTS service-local UI state — a generic JSON k/v bucket.

Same storage mechanism as ``presets`` (the tts service's own ``config`` KV
table in ``.awm/services/tts/tts.db``), but pointed at a different key prefix
and with no built-in / last-used niceties. Each key holds a single JSON value
of arbitrary shape; consumers JSON-encode on the way in and JSON-decode on the
way out.
...
from awm.tts.presets import get_config, set_config

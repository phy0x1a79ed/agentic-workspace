...
Keys live in the tts service's OWN ``config`` KV table
(``.awm/services/tts/tts.db``):

* ``tts_preset:<name>`` → JSON ``{engine, params}``
* ``tts_last_used``     → JSON ``{engine, params}``

Built-in presets are flagged at read-time from an in-module set so seeding
can re-run idempotently and deletion can be refused without a schema change.

Both reads and writes go through this module's ``get_config`` / ``set_config``,
which target the tts service DB (``get_connection("tts")``) — NOT the shared
``config`` service. ``hub_adapter._on_start`` creates the table.
"""
...
def get_config(key: str, default: str | None = None) -> str | None:
    """Read one value from the tts service's own ``config`` KV table."""
...
def set_config(key: str, value: str) -> None:
    """Upsert one value into the tts service's own ``config`` KV table."""

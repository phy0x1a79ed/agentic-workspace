...
from awm.tts.voice.config import EngineRef
...
def _bootstrap() -> None:
    from awm.tts.voice.engines.tts import (  # noqa: F401
        piper, kokoro_rvc, f5tts, gptsovits, sbv2,
    )
...
# Engines dropped into ``$AWM_DATA_DIR/voice/engines/tts/*.py`` are picked up
# without restarting the service. A background asyncio loop scans every 2s and
# (re)registers modules whose mtime changed; files that disappear unregister
# their engine id.

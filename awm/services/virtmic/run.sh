#!/usr/bin/env bash
# run.sh — self-contained entry point for the virtmic service.
#
...
#
# This service owns no listener and no port — the gateway registration buys
# supervision plus the virtmic_status/virtmic_ensure verbs, and the real work
# is a background loop that keeps PulseAudio and the `virtmic` null-sink alive.
#
...
set -euo pipefail
cd "$(dirname "$0")"
MODULE="awm.virtmic.hub_adapter"

"""Tests for the fleet observe plane (absorbed from notifications).

Covers the transcript classifier/accounting (``awm.agents.accounting``), the
report lifecycle + roster (``awm.agents.roster``), and the fleet config contract
(``awm.agents.fleet_config``). All on an isolated agents DB in a temp dir.

Ported verbatim from the retired notifications service's test_notifications.py:
table names gain the ``fleet_`` prefix, and module paths move
service→roster / classify+config→accounting+fleet_config.
"""
...
# ---------------------------------------------------------------------------
# Fixtures — isolated agents DB in a temp dir
# ---------------------------------------------------------------------------
...
def isolated_db(tmp_path, monkeypatch):
    ...
    import awm.agents.dao as daomod
...
def _report(conn, **event):
    ...
    return asyncio.run(roster.handle_report(conn, event))
...
class TestIsQuestion:
    def test_trailing_question_mark(self):
        from awm.agents.accounting import is_question
...
class TestTranscriptRead:
    async def test_reads_last_assistant_text(self, tmp_path):
        from awm.agents.accounting import read_last_assistant
        p = tmp_path / "t.jsonl"
        lines = [
...
class TestReport:
    def test_turn_end_is_always_idle_with_grace(self, conn, tmp_path):
        ...
        from awm.agents.roster import IDLE_GRACE_S
        p = tmp_path / "t.jsonl"
...
class TestSweepAndVerbs:
    def test_stale_session_expires_on_list(self, conn):
        ...
        conn.execute("UPDATE fleet_sessions SET last_seen = ? WHERE session_id = 's9'",
                     (time.time() - roster.STALE_TTL_S - 60,))
        conn.commit()
        out = roster.list_items(conn)
...
class TestUsageAccounting:
    def test_accumulate_categories_and_context(self, tmp_path):
        from awm.agents.accounting import accumulate_usage
        p = tmp_path / "t.jsonl"
        p.write_text(
    ...
    def test_eoot_math_and_unknown_model(self):
        from awm.agents.accounting import eoot, DEFAULT_RATES
...
class TestFleetRoster:
    ...
    def test_non_tmux_session_not_attachable(self, conn):
        ...
        row = [s for s in roster.list_fleet(conn)["sessions"]
    ...
    def test_liveness_window_excludes_old(self, conn):
        ...
        conn.execute("UPDATE fleet_sessions SET last_seen = ? WHERE session_id='f3'",
                     (time.time() - 10_000,))
        conn.commit()
        out = roster.list_fleet(conn, window_s=100)
    ...
    def test_ended_session_drops_on_short_window(self, conn):
        ...
        from awm.agents import roster
        from awm.agents import fleet_config
        ended_win = fleet_config.FLEET_CONTRACT.load_model().ended_window_s
        ...
        conn.execute("UPDATE fleet_sessions SET last_seen = ? WHERE session_id='dead'",
                     (time.time() - ended_win - 60,))
        conn.commit()
        ids = {s["session_id"] for s in roster.list_fleet(conn)["sessions"]}
        assert "live" in ids
    ...
    def test_list_fleet_surfaces_notifications_flag(self, conn):
        from awm.agents import roster
        cfg = roster.list_fleet(conn)["config"]
...
class TestSpawnPlaceholder:
    ...
    def test_real_hook_adopts_placeholder_no_dupe(self, conn):
        ...
        rows = [s for s in roster.list_fleet(conn)["sessions"]
    ...
    def test_cwd_adoption_bounded_by_window(self, conn):
        ...
        roster.SPAWN_ADOPT_WINDOW_S  # sanity: constant exists
        _report(conn, harness="claude", event="spawned",
                session_id="fleet-x-5", tmux_session="fleet-x-5", cwd="/w/shared")
        # Age the placeholder past the window.
        conn.execute(
            "UPDATE fleet_sessions SET first_seen = first_seen - ? WHERE session_id = ?",
            (roster.SPAWN_ADOPT_WINDOW_S + 60, "fleet-x-5"))
        _report(conn, harness="claude", event="session_start",

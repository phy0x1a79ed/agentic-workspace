...
# ---------------------------------------------------------------------------
# Voice WS room_id plumbing — unit-level, no live WS
# ---------------------------------------------------------------------------

def test_run_voice_ws_session_sets_bound_room(monkeypatch):
    """``run_voice_ws_session`` stores room_id on the agent so completed
    turns can be mirrored into the room."""
    import asyncio
    from awm.voice import registry as vreg
    ...
    class FakeRegistry:
        async def get_or_create(self, user_as):
            captured["user_as"] = user_as
            return fake_agent

        async def attach(self, agent, ws):
            captured["attached"] = (agent, ws)
            # Raise immediately to short-circuit the WS receive loop.
            raise RuntimeError("stop-after-attach")

        def detach(self, agent, ws):
            captured["detached"] = (agent, ws)

    monkeypatch.setattr(vreg, "get_registry", lambda: FakeRegistry())

    async def go():
        try:
            await vreg.run_voice_ws_session(
                websocket=object(),
    ...
    asyncio.run(go())
    assert fake_agent.bound_room_id == "room-abc"
    assert captured["user_as"] == "user:operator"

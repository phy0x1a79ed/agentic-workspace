...
<style>
  /* Styles are :global so the same classnames can be applied to spans
     created imperatively by PttTab's contenteditable. */
  :global(.chunk) {
    display: inline-flex;
    align-items: center;
    gap: 0;
  ...
  :global(.chunk-del:hover) { color: var(--danger); }

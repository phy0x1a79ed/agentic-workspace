...
A third changes *how you manage context*: **`reflection`** acts on your own tmux pane. `reflection(verb="compact", args={followup: "<next task>"})` queues a `/compact` behind your current turn plus a follow-up prompt, so the session compacts at end-of-turn and resumes with fresh context instead of going idle — the answer to a filling context mid-task, not a reason to stop. Your pane is detected automatically (the `awm-mcp` proxy in front of you forwards it, with a same-server auto-detect fallback) — you never need to find or pass it. Outside tmux (bare terminal, IDE, web) it returns a clear error rather than a silent no-op. Compact at a clean seam — a phase boundary, a finished chunk, crossing from planning into execution — and pass a `followup` naming the next task so the fresh context resumes pointed at the right work.

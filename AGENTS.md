...
- **Graceful teardown (T4, `server.py` lifespan).** On SIGTERM the lifespan-shutdown half sets a `supervisor.is_shutting_down()` flag (also set early via a SIGTERM-handler wrap so the crash watchdog can't race the teardown), enqueues `shutdown` frames to live services, waits for them to drop leases, force-kills journaled stragglers, and **clears the journal** so the next boot bootstraps clean. **uvicorn caveat:** uvicorn closes the control WSs *before* running lifespan shutdown, so in practice the in-band frame reaches no one (the leases are already released) and the **force-kill-stragglers step is what actually stops the services** — the end-state (zero survivors, journal cleared, watchdog suppressed) is correct either way. dev `do_stop` is gateway-first so this graceful path runs, with `_stop_services` only a backstop sweep.
...
| `page` | static bundle at `/ui/<name>` | `awm/pages/*` via `awm packages sync` |
...
The first-class way to add a backend is a folder under `awm/services/<name>/` with a self-contained executable `run.sh`. The gateway **discovers** it by filesystem scan (`awm/gateway/awm/gateway/hub/discovery.py`), **bootstraps** it on first boot, respawns it durably, and injects **exactly three** env vars (`AWM_HUB_URL`, `AWM_SERVICE_NAME`, `AWM_SERVICE_ID`) — no auth, ever. Enable/disable state lives in `.awm/services/enabled.json`; the operator surface is `awm services list|start|stop|restart|enable|disable [name|--all]`. This **supersedes** the old `kind=stripe` / `packages/services/*` stripe-authoring flow (the `svc-*` + `comp-*`-registered-via-`awm hub register` vertical-stripe model). The full human-facing authoring contract is **README § *Authoring a service***.

The `url` / `static` kinds and the `awm hub register --url|--dir` lease commands remain only for **external** bundles/services (a remote HTTP/WS upstream, or a hand-built static dir) — not for awm's own backends, which are `awm/services/*` folders.
...
For an **external** upstream the gateway should front, register a lease via `awm hub register`. These POST/DELETE/WS to `/hub/*` on the gateway origin; useful as the raw API when scripting:
...
`awm packages gen → npm install → build` page pipeline, and the hub-service
...
etc. Where README.md tells you how to use the system, the rest of this file
...
- **CLI** — `awm services *`, `awm packages gen/sync/list`, and `awm dev shadow` live in `awm/gateway/awm/gateway/cli.py` (search for `services_app` / `packages_app` / `dev_app`).

...
**Remaining (not done): the modular tree has NOT been installed into the prod `awm` env** — the cutover (run `awm/gateway/install.sh`, then `python -m awm.scopes.seed <live state.db>` to migrate the live DB, then flip `awm.service`) is the open step. Key foundation facts for anyone touching this:

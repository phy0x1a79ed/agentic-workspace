...
- **Feature services** (`awm/services/<name>/`, each its own dist + DB + `run.sh`) — `scopes`, `agents`, `artifacts`, `skills` (retired/disabled — debrief is now a native CC skill; the rest of the catalog is reference-only on disk), `discord`, plus the voice pair `stt`/`tts`.

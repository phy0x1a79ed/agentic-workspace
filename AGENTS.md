...
**Shared data is not a `userdata` scope at all** — it is its own project. Every subdirectory of `projects/userdata/` is taken for a person by `userroot.users()`, so a shared store put there becomes a phantom user. The Trilium vault is the worked example: one instance for everyone, at project `vault`, and the awm edge serves it at `/trilium/` behind the ordinary session. There was briefly a third shape — one process per person, at scope `<service>/<name>` — built when the upstream application's own login was the only thing that could tell two people apart. It is retired: it made adding a person a DNS and web-server job, and once the edge had per-user accounts the second login was asking a question it had already answered.

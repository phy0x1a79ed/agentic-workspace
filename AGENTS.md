...
awm is being split from the monolithic `awm/` package into a nested tree of pip dists under `awm/`: the **gateway** (`awm/gateway/`, package `awm.gateway`) — the sole interface + coordination hub — plus shared components (`awm.config`, `awm.persistence`, `awm.gatewayclient`) and self-contained feature services (`awm.scopes`, `awm.agents`, `awm.artifacts`, `awm.skills`, `awm.discord`). The **gateway boots standalone** (de-DB'd — it owns no tables); feature surfaces return only as those modules migrate and register.

Migration status: the **foundation pass is done** and **`discord` is migrated** as the proven reference (register→catalog→dispatch verified end-to-end). `scopes`/`agents`/`artifacts`/`skills` are not yet fanned out. Key foundation facts for anyone touching this:

...
- **Shared Python components** (`awm/service_components/`, pure imported source, no `install.sh`) — `awm.config`, `awm.persistence`, `awm.gatewayclient`, `awm.agentcore`, `awm.claudedaemon`.
...
- **Services are OUT-OF-PROC**, spawned + PID-journaled by the supervisor; the control-WS lease is liveness. Each service is a self-contained `run.sh` (the only thing the gateway runs — `bash run.sh`) plus a tiny `hub_adapter.py` built on **`awm.gatewayclient.ServiceAdapter`** (the reusable register→ready→serve→dispatch→reconnect base). `awm.gatewayclient` also carries `call`/`call_sync`/`RefCache` for the service→service direction. The `/svc/*` control plane is unauthenticated (loopback only). `awm/services/dev/` is the worked example to copy: the smallest folder that still ships every required piece.
- **Install/iterate each module via its own `install.sh`, never hand-rolled `pip`.** `awm/gateway/install.sh` is the composition root — it installs `config` + `persistence` and every feature dist `--no-deps`, then the gateway itself (resolving third-party deps). Imported-source components (`config`, `persistence`, `gatewayclient`, `agentcore`, `claudedaemon`, `ui_components`) have no `install.sh`. Override the target env with `AWM_ENV`.
...
- **Dual `/tools` projection — expanded vs collapsed-by-domain.** `catalog.list_tools()` is the **expanded** per-verb surface (~71 tools); the CLI generator (`register_service_cli_commands`) and the flat `/invoke` by-name dispatch both depend on it, so it stays. Alongside it, `catalog.list_domain_tools()` folds that same surface **by domain** — the projected name split on the first underscore (`scope_create` → domain `scope` / verb `create`; native ops fold by `cli_group`/`cli_command`) — into one generic `{verb, args}` tool per domain (~8–10 total). `GET /tools?view=domains` returns the collapsed view; the **MCP stdio proxy requests it** (so every MCP client, including non-deferring spawned agents + OpenCode, carries the tiny surface), while the default `GET /tools` and the CLI/HTTP surfaces stay expanded. Adding **`&peers=1`** widens that same collapse to the fleet — still one tool per domain name, but peer-only domains appear and the envelope's third key `peer` chooses the node (see FEDERATION.md § *Cross-peer calls*). It is opt-in for a reason: the plain view is what a *peer* fetches from this node's edge, and it must stay local-only or the fleet advertises transitive peers nobody can dial. `dispatch()` gained a domain branch *ahead* of the flat branches: when `name` is a known domain and `args` carries `verb`, it routes to `_dispatch_domain` — `verb="describe"` is answered from the catalog (no service round-trip; `describe` is a **reserved verb** on every domain), a native verb runs its `Operation`, and a service verb is resolved back to its internal fn via the existing `_find_service_fn(f"{domain}_{verb}")` reverse lookup (so a name≠tool divergence like `scope_refresh`→`awm_refresh` still routes) and RPC'd with `as_` threaded as before. The collapse is **purely additive** in the gateway — flat dispatch, the default `/tools`, and all CLI/HTTP generation are untouched, so it's reversible by reverting the proxy's `?view=domains` request. A domain's verbs must be unique (folding warn-and-skips dups, first-wins) — keep new `"tool"` overrides under a real `<domain>_<verb>` shape (a bare single-token override becomes its own junk single-verb domain, and a **two-word service name splits at its first underscore** — `claude-science` projecting `claude_science_status` lands as domain `claude` / verb `science_status`, so such a service must pick a one-token domain: `science_status`, giving `awm science status`). A manifest function may carry `"surfaces": ["cli","http"]` (default = all three when omitted) to keep a verb **off the MCP surface**: `_domain_catalog` skips non-`mcp` functions and `_dispatch_domain` rejects them, so `writing(verb="add")` is unknown/404 over MCP while the CLI's flat `/invoke {name:"writing_add"}` (via `_find_service_fn`, unfiltered) still works — the mechanism behind CLI-only write verbs (`awm/services/writing`, and 2fa's arming/approval verbs). A manifest may also carry a **top-level `"description"`** — prose about how the domain is meant to be used, which `_domain_blurbs` prepends to the otherwise fully-generated domain tool description and adds to `describe`'s reply; it is the only channel a service has for guidance an agent reads *before* choosing a verb (`2fa` uses it to send callers to `ssh(verb=connect)`), and it is this node's only, since a peer's blurb lives in that peer's catalog. A function's `"timeout"` is now honored on the `/invoke`/domain path too (via `_rpc_call` → `_fn_timeout`), not only the `/svc/<name>/fn/<fn>` proxy — see the timeout ladder below for what the clients do with it.
- **The timeout ladder, and the rule it encodes: an unanswered request is not an unreachable daemon.** Four numbers answer four different questions, and collapsing any two is how a *successful* `scope create` came back as `awm daemon unreachable after 10.0s: timed out` for months. (1) A manifest function's `"timeout"` is the authoritative budget — how long the verb may legitimately run — and the gateway enforces it server-side. (2) A client's **read ceiling** is a backstop on top of that, never a budget: `mcp_http.DEFAULT_READ_TIMEOUT` (env `AWM_MCP_READ_TIMEOUT`), shared by both MCP proxies and the generated CLI dispatch, and deliberately above the largest declared budget (3600s, the dvc `wait` verb) so it can never fire first. (3) `GET /tools` gets its own short ceiling — it blocks session startup and the core answers it in ~19ms. (4) The **reconnect window** (10s) is a different question entirely — *is the daemon coming back?* — and is meaningful only before the request lands. Applying it to a reply already owed was the bug: urllib raises a bare `TimeoutError` (an `OSError`, hence swallowed by a catch-all) in the response phase and wraps connect/send failures in `URLError`, so **retry the connect phase and never the response phase** — `/invoke` is not idempotent and by then the core is already running the call. It is running it because nothing on that path learns the client left: `/invoke` has no disconnect awareness and `ControlChannel.call` enqueues onto the service's control WS *before* it awaits. A response-phase timeout therefore raises `mcp_http.CoreNoReply`, whose envelope tells the caller the request was delivered and to re-check with a read verb rather than retry. Both proxies share `mcp_http` for the same reason `peer_files` is its own module — the `AWM_MCP_SDK=1` rollback had silently drifted, omitting `httpx.ReadTimeout` from its retry tuple so the same failure surfaced there as `{"error": ""}`. **A change here reaches agents only after their MCP clients restart.**
...
- **A child that must outlive awm needs a different cgroup, not just a detach.** Where the gateway runs under systemd (prod), `systemctl restart awm` kills by control group, and a cgroup is inherited by every descendant however it forks, `setsid`s or double-forks. Detaching defeats signal-based teardown and nothing else. A service spawning something meant to survive a deploy has to place it outside `awm.service` — `systemd-run --user` into a transient unit is the mechanism `awm/services/claude-science` uses. The inverse trap is `awm/services/cx`'s: a Claude Code background session inherits the cgroup *and the environment* of whichever process first started the node's `claude daemon`, so a service that launches one when no daemon is up donates awm's cgroup to every session on the node. It refuses to seed unless a daemon is already running, which is the protection; the transient unit is only the backstop. The failure is invisible in dev and silent in prod: the process simply is not there after the next restart.
...
- **A collection is only as good as the directory it runs from.** `dvc gc`
  deletes from the cache its *working directory* resolves to, and unions the
  keep-set hashes across every repo named in `--projects`. So the cwd is chosen
  (first named worktree whose `dvc cache dir` matches the shared cache) rather
  than inherited from the head of a sorted list — a worktree awm never
  provisioned has no `.dvc/config.local`, resolves *upward* to some other cache
  rather than failing, and the run then reports "no unused cache to remove"
  about a cache it never opened. That cwd repo is then **excluded** from
  `--projects`: dvc locks the working directory *and* every listed repo, so
  naming it twice deadlocks against itself as "Unable to acquire lock" (which
  is not a stale lock — those are `flock`s the kernel drops when the holder
  dies, and every lock file in this workspace is a dead holder's). It stays in
  the keep set regardless.
...
- **MCP proxies** — `mcp_server.py` picks between `mcp_stdio.py` (default) and `mcp_server_sdk.py` (`AWM_MCP_SDK=1` rollback); `mcp_http.py` holds the timeout ladder and failure taxonomy both must agree on.
...
**Per-user data is a scope too**, in project `userdata`. A service that partitions **by caller** serves every person from one process and resolves who is asking per request: `awm.config.userroot` (`resolve(as_)` → the user or `None`, `root_for(user)` → that worktree, `state_dir(service, user)` → its own index), committing its subdirectory with `awm.config.autocommit`, with `userroot.wrap_handlers` binding the caller for every verb. Notes and drawio are the reference implementations and live at scope `<name>`, branch `user/<name>`.

**Shared data is not a `userdata` scope at all.** Every subdirectory of `projects/userdata/` is taken for a person by `userroot.users()`, so a shared store put there becomes a phantom user. It belongs in a scope of the project whose code serves it, DVC-pinned under `data/`, so that one path moves on a deploy instead of two. The Trilium vault is the worked example: one instance for everyone, its database and its pinned chunks in `projects/trilium/release` beside the fork that serves them, and the awm edge serving it at `/trilium/` behind the ordinary session. It was a project of its own until that cost more than it bought. A node that runs the service without a checkout of that project gets a plain directory at the same path and pins nothing — the path has to match, the shape does not. There was briefly a third shape — one process per person, at scope `<service>/<name>` — built when the upstream application's own login was the only thing that could tell two people apart. It is retired: it made adding a person a DNS and web-server job, and once the edge had per-user accounts the second login was asking a question it had already answered.

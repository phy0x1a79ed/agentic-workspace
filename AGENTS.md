...
Migration status: the **foundation pass + all five feature modules are migrated**. `discord` was the proven reference; `skills`/`scopes`/`agents`/`artifacts` followed it onto their own per-service DBs + DAO + seed + `hub_adapter` on `ServiceAdapter`. Verified end-to-end in an isolated workspace (`/tmp/awm_modtest_full.sh`): all 5 services register → 60 ops project into `/tools` → the cross-service identity path is live (`ensureProject`→`ensureScope`→`resolveScope`; `artifacts.register` RPC-validates `(project, scope)` against the live `scopes` service before writing). `setup.sh` is repointed at `awm/gateway/install.sh` (the composition root). Per-dist tests green (skills 26 / scopes 132 / agents 37 / artifacts 16, run with the dist roots on `PYTHONPATH`). **Remaining monolith-retirement work** (not yet done): the legacy `awm/tests/` tree still holds the pre-fanout test files (feature tests were re-homed as copies into each dist; gateway-owned `hub`/`mcp`/`auth`/`unit` tests still live there) and the repo-root `pyproject.toml` `testpaths` still points at `awm/tests`; the `scopes`→`agents` `roomAgentsKillOnClose` reverse-notification (kill agents on room close) is deferred pending a pub/sub channel; the modular tree has NOT been installed into the prod `awm` env. Key foundation facts for anyone touching this:
